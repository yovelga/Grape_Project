...
def extract_HS_folders_by_date_and_tag(base_path, dest_path):
    ...
    expected_tags = [f"1_{i:02d}" for i in range(1, 61)] + [f"2_{i:02d}" for i in
                                                            range(1, 61)]  # 1_01 to 1_60, 2_01 to 2_60
    ...
    for date_folder in tqdm(date_folders, desc="Processing dates", unit="date"):
        ...
        if os.path.exists(hs_path) and os.path.isdir(hs_path):
            ...
            for image_folder in tqdm(image_folders, desc=f"Processing folders in {date_folder}", unit="folder",
                                     leave=False):
                ...
                if os.path.isfile(xml_file_path):
                    try:
                        ...
                        if global_tag_value and (
                                '0101' <= global_tag_value <= '0160' or '0201' <= global_tag_value <= '0260'):

                            formatted_date = date_folder.replace('-', '.')
                            tag_prefix = '1_' if '0101' <= global_tag_value <= '0160' else '2_'
                            tag_folder_name = tag_prefix + global_tag_value[-2:]
                            present_tags.append(tag_folder_name)

                            destination_folder = os.path.join(dest_path, tag_folder_name, formatted_date, 'HS')
                            ...
                            try:
                                ...
                                for item in os.listdir(src_folder_path):
                                    ...
                                    if os.path.isdir(s):
                                        ...
                                        shutil.copy2(s, d)
                                summary_data.setdefault(tag_folder_name, {}).update({formatted_date: 'succeeded'})
...
def save_summary_to_csv(summary_data, dest_path):
    ...
    with open(csv_file_path, mode='w', newline='') as file:
        ...
        for tag, date_info in summary_data.items():
            row = [tag] + [date_info.get(date, 'missing') for date in
                           all_dates]  # Use 'missing' if no data for the date
            writer.writerow(row)
