...
def clean_checkpoint(ckpt_path):
    ...
    unexpected_keys = ["no_obj_embed_spatial", "obj_ptr_tpos_proj.weight", "obj_ptr_tpos_proj.bias"]
...
def initialize_sam2():
    ...
    if not GlobalHydra.instance().is_initialized():
        hydra.initialize(config_path="segment-anything-2/sam2/configs/sam2", version_base=None)


    print("✅ HYDRA CONFIG PATH SET:", "/storage/yovelg/Grape/MaskGenerator/segment-anything-2/sam2/configs/sam2")

    from sam2.build_sam import build_sam2
    from sam2.automatic_mask_generator import SAM2AutomaticMaskGenerator

    cleaned_weights = clean_checkpoint(SAM_WEIGHTS)  # השתמש בקובץ הנקי
    sam2_model = build_sam2("sam2_hiera_l", cleaned_weights, device=DEVICE, apply_postprocessing=False)
