...
def read_excel_or_csv(path):
    ...
    for item in os.listdir(path):
        if item.endswith('.csv'):
            ...
            return pd.read_csv(os.path.join(path, item), dtype={'Thermo ID  (4 Digits)': str, 'RGB  ID  (4 Digits)': str})
        elif item.endswith('.xlsx'):
            # print(f"Found Excel file: {item}")
            return pd.read_excel(os.path.join(path, item), dtype={'Thermo ID  (4 Digits)': str, 'RGB  ID  (4 Digits)': str})
...
def extract_images(date, source, dest, grapes_id, thermos_id, rgbs_id, rgb_summary, thermal_summary):
    for index in tqdm(range(len(grapes_id)), desc=f"Processing Grape IDs for {date}"):
        ...
        for img in os.listdir(rgb_source_path):
            if img.endswith(f'{rgb_id}.JPG'):  # Assuming images are .JPG
                ...
                copy_image(os.path.join(rgb_source_path, img), os.path.join(destination_folder, 'RGB'))
                rgb_count += 1
        ...
        for img in os.listdir(thermo_source_path):
            ...
            if file_id == thermo_id:
                destination_folder = os.path.join(dest, grape_id, date)
                copy_image(os.path.join(thermo_source_path, img), os.path.join(destination_folder, 'Thermal'))
                thermo_count += 1
...
def main():
    ...
    for date in tqdm(folders_df['Folder Name'], desc="Processing Dates"):
        ...
        extract_images(date, base_path, destination_path, grapes_id, thermos_id, rgbs_id, rgb_summary, thermal_summary)
    ...
    thermal_pivot = thermal_df.pivot(index='Grape ID', columns='Date', values='Count').fillna(0)
